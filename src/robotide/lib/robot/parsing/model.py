...
class _SettingTable(_Table, _WithSettings):
    ...
    def __init__(self, parent):
        ...
        self.doc = Documentation('文档', self)
        self.suite_setup = Fixture('Suite Setup', self,title='集设置')
        self.suite_teardown = Fixture('Suite Teardown', self,title='集卸载')
        self.test_setup = Fixture('Test Setup', self,title='测试配置')
        self.test_teardown = Fixture('Test Teardown', self,title='测试卸载')
        self.force_tags = Tags('Force Tags', self,title='强制标签')
        self.default_tags = Tags('Default Tags', self,title='默认标签')
        self.test_template = Template('Test Template', self,title='测试模版')
        self.test_timeout = Timeout('Test Timeout', self,title='测试超时')
